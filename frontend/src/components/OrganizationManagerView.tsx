...
export const OrganizationManagerView: React.FC<OrganizationManagerViewProps> = ({
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-brand-900 via-indigo-950 to-navy-950 p-6 sm:p-8 text-white shadow-md space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-brand-300 text-xs font-semibold border border-white/10">
              ...
              <span>Multi-Tenant Organization</span>
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 text-white font-medium text-xs backdrop-blur-md border border-white/20 transition-all flex items-center gap-2"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-white/10 text-xs">
          <div>
            ...
            <div className="font-semibold text-white text-sm mt-0.5">{activeOrg.plan}</div>
          </div>

          <div>
            <div className="text-slate-400 font-medium">Active Members</div>
            <div className="font-semibold text-white text-sm mt-0.5">{orgMembers.length} Members</div>
          </div>

          <div>
            <div className="text-slate-400 font-medium">Department Teams</div>
            <div className="font-semibold text-white text-sm mt-0.5">{activeOrg.teams.length} Teams</div>
          </div>

          <div>
            <div className="text-slate-400 font-medium">Isolated Storage</div>
            <div className="font-semibold text-white text-sm mt-0.5">{(activeOrg.storage_used_mb / 1024).toFixed(1)} GB / {(activeOrg.max_storage_mb / 1024).toFixed(0)} GB</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 p-6 rounded-xl space-y-6 border border-slate-200 dark:border-white/[0.06]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {activeOrg.teams.map((team) => (
            <div
              key={team.id}
              className="p-5 rounded-lg bg-slate-50 dark:bg-white/[0.04]/60 border border-slate-200/60 dark:border-white/[0.06] space-y-3 hover:border-brand-500/40 transition-colors"
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-brand-500/10 text-brand-600 dark:text-brand-400 border border-brand-500/15">
                  {team.name}
                </span>
              ...
              <div className="text-[11px] text-slate-400 flex items-center justify-between pt-2 border-t border-slate-200/50 dark:border-white/[0.06]">
      ...
      <div className="bg-white dark:bg-gray-900 p-6 rounded-xl space-y-6 border border-slate-200 dark:border-white/[0.06]">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => setIsInviteOpen(true)}
            className="px-3.5 py-2 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-medium text-xs shadow-md transition-all flex items-center gap-1.5"
          >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-slate-200 dark:border-white/[0.06] text-slate-400 font-medium uppercase text-[11px]">
                ...
                return (
                  <tr key={member.id} className="hover:bg-slate-50 dark:hover:bg-navy-900/50 transition-colors">
                    ...
                    <td className="py-3.5 px-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-brand-600 to-indigo-600 text-white font-medium flex items-center justify-center text-xs">
                    ...
                    <td className="py-3.5 px-4">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold border ${rInfo.bg}`}>
                    ...
                    <td className="py-3.5 px-4">
                      <div className="flex flex-wrap gap-1">
                        {member.team_ids.map(tid => {
                          ...
                          return (
                            <span key={tid} className="px-2 py-0.5 rounded bg-slate-200 dark:bg-white/[0.05] text-slate-700 dark:text-slate-300 text-[11px] font-semibold">
                              {t?.name || tid}
                            </span>
                          );
                        })}
      ...
      <div className="bg-white dark:bg-gray-900 p-6 rounded-xl space-y-4 border border-slate-200 dark:border-white/[0.06]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
          ...
          <div className="p-3.5 rounded-lg bg-slate-50 dark:bg-white/[0.04]/60 border border-slate-200/50 dark:border-white/[0.06] space-y-1">
            <div className="font-medium text-brand-600 flex items-center gap-1">
      ...
      {isInviteOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md bg-white dark:bg-gray-900 rounded-xl p-6 space-y-6 border border-slate-200 dark:border-white/[0.06] shadow-lg">
            ...
            <form onSubmit={handleInviteSubmit} className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 bg-slate-100 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] rounded-xl text-xs"
                />
              </div>

              <div className="space-y-1">
                <label className="text-xs font-medium text-slate-700 dark:text-slate-300">Member Full Name</label>
                <input
                  type="text"
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  className="w-full px-3 py-2.5 bg-slate-100 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] rounded-xl text-xs font-medium"
                >
              ...
              <div className="space-y-1">
                <label className="text-xs font-medium text-slate-700 dark:text-slate-300">Assign to Department Teams</label>
                <div className="space-y-1">
              ...
              <button
                type="submit"
                className="w-full py-3 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-medium text-xs shadow-md transition-all"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md bg-white dark:bg-gray-900 rounded-xl p-6 space-y-6 border border-slate-200 dark:border-white/[0.06] shadow-lg">
            ...
            <form onSubmit={handleCreateTeamSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-medium text-slate-700 dark:text-slate-300">Department Team Name</label>
                <input
                  type="text"
                  required
              ...
              <div className="space-y-1">
                <label className="text-xs font-medium text-slate-700 dark:text-slate-300">Team Description</label>
                <input
                  type="text"
