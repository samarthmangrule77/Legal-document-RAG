      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-slate-900 via-navy-950 to-indigo-950 p-8 border border-slate-800 text-white shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            ...
            <h1 className="text-3xl font-extrabold tracking-tight">AI Legal Workflow Automation Builder</h1>
          ...
          <button
            ...
            disabled={isRunning}
            className={`px-6 py-3.5 rounded-2xl font-black text-sm flex items-center gap-2.5 shadow-xl transition-all ${
              isRunning
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-slate-400">Target Doc:</span>
                <select
                  value={doc?.id}
                  onChange={(e) => {
                  ...
                  <div key={step.id} className="relative">
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">{step.category}</span>
                            {step.duration && (
                              <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-[10px] font-mono text-emerald-600 dark:text-emerald-400 font-bold">
                                ⏱ {step.duration}
                              </span>
                            )}
                          </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-4">
            ...
            <h3 className="text-base font-black text-slate-900 dark:text-white">Legal Team Email Digest</h3>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 text-xs font-semibold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-amber-500/40"
          ...
          <div className="p-6 rounded-3xl bg-slate-950 text-slate-200 border border-slate-800 shadow-xl space-y-4">
            ...
            <div className="font-mono text-[11px] space-y-2 max-h-64 overflow-y-auto leading-relaxed text-slate-300">
              {executionLogs.length === 0 ? (
                ...
                executionLogs.map((log, idx) => (
                  <div key={idx} className={log.includes('[SUCCESS]') || log.includes('[FINISHED]') ? 'text-emerald-400 font-bold' : log.includes('[RUNNING]') ? 'text-amber-400' : 'text-slate-300'}>
                    {log}
                  </div>
                ))
              )}
