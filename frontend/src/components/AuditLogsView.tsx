...
export const AuditLogsView: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-12 animate-fade-in">
      ...
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-slate-900 via-navy-950 to-indigo-950 p-8 border border-white/[0.06] text-white shadow-lg">
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            ...
            <h1 className="text-3xl font-semibold tracking-tight">Audit Logs & Telemetry</h1>
          ...
          <div className="flex items-center gap-2">
            <span className="px-3.5 py-2 rounded-xl bg-slate-800/80 border border-white/[0.08] text-xs font-mono font-medium text-emerald-400 flex items-center gap-2">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-6 rounded-xl bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] shadow-sm">
        ...
        <div className="relative w-full md:w-96">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 text-xs font-semibold bg-slate-50 dark:bg-white/[0.05] border border-slate-200 dark:border-white/[0.08] rounded-xl text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-brand-500/40"
          />
        </div>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all whitespace-nowrap ${
                selectedCategory === cat
                  ...
                  : 'bg-slate-100 dark:bg-white/[0.05] text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'
              }`}
            >
      ...
      <div className="p-8 rounded-xl bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] shadow-sm space-y-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-slate-200 dark:border-white/[0.06] text-slate-400 font-semibold uppercase tracking-wider">
              ...
              {filteredLogs.map((log) => {
                ...
                return (
                  <tr key={log.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="py-4 px-3 font-medium text-slate-900 dark:text-white whitespace-nowrap">
                      <div>{log.user_name}</div>
                      <div className="text-[11px] text-slate-400 font-normal font-mono">{log.user_email}</div>
                    </td>
                    <td className="py-4 px-3 whitespace-nowrap">
                      <span className={`px-2.5 py-1 rounded-full text-[11px] font-semibold border flex items-center gap-1.5 w-fit ${badge.bg}`}>
                        <Icon className="w-3 h-3" />
                        <span>{log.action_type}</span>
                      </span>
                    </td>
                    <td className="py-4 px-3 font-medium text-brand-600 dark:text-brand-400 whitespace-nowrap">
                      {log.target_resource}
                    </td>
