...
export const BillingDashboardView: React.FC<BillingDashboardViewProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      ...
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-slate-900 via-navy-950 to-indigo-950 p-8 border border-white/[0.06] text-white shadow-lg">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            ...
            <h1 className="text-3xl font-semibold tracking-tight">Billing & SaaS Subscription</h1>
          ...
          <div className="flex items-center gap-3 bg-slate-800/80 backdrop-blur border border-white/[0.08]/80 p-3.5 rounded-lg">
            ...
            <div>
              ...
              <div className="text-base font-medium text-white flex items-center gap-2">
                ...
                <span className={`px-2 py-0.5 text-[11px] font-medium rounded-full ${isFreePlan ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30'
                  }`}>
      ...
      {successMsg && (
        <div className="flex items-center justify-between p-4 rounded-lg bg-emerald-500/15 border border-emerald-500/30 text-emerald-700 dark:text-emerald-300 text-sm font-semibold animate-in fade-in slide-in-from-top-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="p-6 rounded-xl bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] shadow-sm flex flex-col justify-between">
          ...
          <div className="my-4">
            <div className="flex items-baseline justify-between mb-2">
              ...
              {isFreePlan && (
                <span className={`text-xs font-medium ${isAtLimit ? 'text-rose-500' : isNearLimit ? 'text-amber-500' : 'text-slate-500'}`}>
            ...
            {/* Progress bar */}
            {isFreePlan ? (
              <div className="w-full bg-slate-100 dark:bg-white/[0.05] rounded-full h-3 overflow-hidden p-0.5 border border-slate-200/50 dark:border-white/[0.06]">
                <div
        ...
        <div className="p-6 rounded-xl bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] shadow-sm flex flex-col justify-between">
          ...
          <div className="my-4 space-y-1">
            <div className="text-xl font-medium text-slate-900 dark:text-white flex items-center gap-2">
              Stripe 256-bit SSL
            </div>
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl font-semibold text-slate-900 dark:text-white">Select Your SaaS Tier</h2>
          ...
          <div className="flex items-center bg-slate-200/70 dark:bg-white/[0.05]/80 p-1.5 rounded-lg border border-slate-300/50 dark:border-white/[0.08]">
            <button
              onClick={() => setBillingCycle('monthly')}
              className={`px-4 py-2 rounded-xl text-xs font-medium transition-all ${billingCycle === 'monthly'
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-medium transition-all ${billingCycle === 'annual'
                ? 'bg-white dark:bg-white/[0.03] text-brand-600 dark:text-brand-400 shadow-sm'
                : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                }`}
            >
              Annual Billing
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {plans.map((plan) => {
            ...
            return (
              <div
                key={plan.id}
                className={`relative flex flex-col justify-between p-8 rounded-xl transition-all duration-300 ${isPopular
                  ? 'bg-white dark:bg-white/[0.03] border-2 border-brand-500 shadow-md shadow-brand-500/10 scale-105 z-10'
                  : 'bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] shadow-sm hover:border-slate-300 dark:hover:border-white/[0.08]'
                  }`}
              >
                {/* Popular Badge */}
                {isPopular && (
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-brand-600 text-white text-xs font-semibold uppercase tracking-wider shadow-md">
                    Most Popular Choice
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="p-3.5 rounded-lg bg-slate-50 dark:bg-white/[0.04] border border-slate-200/70 dark:border-white/[0.08]/70 mb-6">
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-center gap-2 py-3.5 px-4 rounded-lg font-medium text-sm transition-all ${isCurrent
                    ? 'bg-slate-100 dark:bg-white/[0.05] text-slate-400 cursor-default border border-slate-200 dark:border-white/[0.08]'
                    : isPopular
                      ? 'bg-brand-600 hover:bg-brand-700 text-white shadow-md active:scale-98'
      ...
      <div className="p-8 rounded-xl bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] shadow-sm space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Invoices & Receipts</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">Download past payment invoices generated via Stripe.</p>
          </div>
          <span className="px-3 py-1 rounded-full bg-slate-100 dark:bg-white/[0.05] text-slate-600 dark:text-slate-300 text-xs font-semibold">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-600 dark:text-slate-400">
            <thead className="bg-slate-50 dark:bg-white/[0.03] text-slate-400 font-medium uppercase tracking-wider text-[11px]">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {invoices.map((inv) => (
                <tr key={inv.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-colors">
                  <td className="px-4 py-3.5 font-mono font-medium text-slate-900 dark:text-slate-200">
                    {inv.id}
                  </td>
                  <td className="px-4 py-3.5">{inv.date}</td>
                  <td className="px-4 py-3.5 font-medium text-slate-700 dark:text-slate-300">{inv.plan}</td>
                  <td className="px-4 py-3.5 font-semibold text-slate-900 dark:text-white">{inv.amount}</td>
                  <td className="px-4 py-3.5">
                    <span className="px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                      {inv.status}
                    </span>
                  </td>
      ...
      {stripeSimulatedModal.open && (
        <div className="fixed inset-0 z-50 bg-gray-950/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] p-8 rounded-xl max-w-md w-full text-center space-y-6 shadow-lg animate-in zoom-in-95">
            <div className="w-16 h-16 rounded-full bg-brand-600 mx-auto flex items-center justify-center text-white shadow-md">
              <Loader2 className="w-8 h-8 animate-spin" />
            </div>
            <div>
              <h3 className="text-xl font-semibold text-slate-900 dark:text-white">Connecting to Stripe Checkout</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
                Securing checkout session for <span className="font-medium text-brand-600">{stripeSimulatedModal.plan?.name}</span>...
              </p>
            </div>
            <div className="p-3 bg-slate-50 dark:bg-white/[0.05] rounded-lg text-left text-xs text-slate-500 space-y-1">
              ...
              <div className="flex justify-between font-medium text-slate-900 dark:text-white pt-1 border-t border-slate-200 dark:border-white/[0.08]">
                <span>Total Due:</span>
      ...
      {showCancelConfirm && (
        <div className="fixed inset-0 z-50 bg-gray-950/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] p-6 rounded-xl max-w-md w-full space-y-4 shadow-lg">
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl text-xs font-medium bg-rose-600 hover:bg-rose-500 text-white shadow-md shadow-rose-500/20"
              >
