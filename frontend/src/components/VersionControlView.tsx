    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-slate-900 via-navy-950 to-indigo-950 p-8 border border-slate-800 text-white shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            ...
            <h1 className="text-3xl font-extrabold tracking-tight">Contract Revision & Redline Diff Tracker</h1>
      ...
      <div className="p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-6">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-bold text-slate-400">Active Selected Version: <span className="text-brand-600 font-mono">{selectedVersion.toUpperCase()}</span></span>
        </div>
          ...
          {VERSIONS_DATA.map((ver) => {
            ...
            return (
              <button
                ...
                className={`text-left p-6 rounded-2xl transition-all duration-200 space-y-3 flex flex-col justify-between ${
                  isSelected
              ...
              >
                <div>
                  <div className="flex items-center justify-between">
                    <span className="px-2.5 py-1 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-black font-mono">
                  ...
                  <h3 className="font-black text-sm text-slate-900 dark:text-white mt-3">{ver.label}</h3>
                ...
                <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs font-bold text-brand-600">
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-mono">
                <div className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/20 text-rose-900 dark:text-rose-200 space-y-1">
                  <div className="font-extrabold text-[10px] uppercase tracking-wider text-rose-600">Contract V1.0 (Original Draft):</div>
                  <p className="line-through leading-relaxed opacity-90">{diff.v1_text}</p>
                </div>

                <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-900 dark:text-emerald-200 space-y-1">
                  <div className="font-extrabold text-[10px] uppercase tracking-wider text-emerald-600">Contract {selectedVersion.toUpperCase()} (Revised):</div>
                  <p className="leading-relaxed font-bold">{diff.v3_text}</p>
                </div>
              </div>

              {/* Legal Analysis Banner */}
              <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 text-xs text-slate-700 dark:text-slate-300 flex items-center gap-2">
