...
const DEFAULT_TEMPLATES: ContractTemplate[] = [
  ...
  },
  {
    ...
    description: 'cloud SaaS agreement outlining service SLA, payment terms, data protection, and liability caps.',
    fields: [
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      ...
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-slate-900 via-navy-950 to-indigo-950 p-8 border border-white/[0.06] text-white shadow-lg">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            ...
            <h1 className="text-3xl font-semibold tracking-tight">AI Contract Generator & Drafter</h1>
          ...
          {DEFAULT_TEMPLATES.map((tmpl) => {
            ...
            return (
              <button
                ...
                className={`text-left p-5 rounded-lg transition-all duration-200 flex flex-col justify-between ${
                  isSelected
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <div className="p-2.5 rounded-xl bg-slate-100 dark:bg-white/[0.05] border border-slate-200 dark:border-white/[0.08]">
                      {getTemplateIcon(tmpl.id)}
                    </div>
                    {isSelected && (
                      <span className="px-2 py-0.5 rounded-full bg-brand-500/15 text-brand-600 dark:text-brand-400 text-[11px] font-medium border border-brand-500/15">
                        Selected
                      </span>
                    )}
                  </div>

                  <h3 className="font-semibold text-sm text-slate-900 dark:text-white">{tmpl.title}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 line-clamp-2 leading-relaxed">
                    {tmpl.description}
                  </p>
                </div>

                <div className="mt-4 pt-3 border-t border-slate-100 dark:border-white/[0.06] flex items-center justify-between text-[11px] font-semibold text-slate-400">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="p-6 rounded-xl bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] shadow-sm space-y-5">
            <div>
            ...
            <form onSubmit={handleGenerateSubmit} className="space-y-4">
              {selectedTemplate.fields.map((field) => (
                <div key={field.name} className="space-y-1.5">
                  <label className="text-xs font-medium text-slate-700 dark:text-slate-300 flex items-center justify-between">
                    ...
                    {field.required && <span className="text-[11px] text-rose-500 font-semibold">*Required</span>}
                  </label>
                  <input
                    type={field.type}
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-white/[0.05]/80 border border-slate-200 dark:border-white/[0.08] rounded-xl text-xs text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500/40"
                  />
                </div>
              ))}
              ...
              <button
                ...
                className="w-full mt-2 flex items-center justify-center gap-2.5 py-3.5 px-4 rounded-lg bg-gradient-to-r from-brand-600 via-brand-500 to-indigo-600 hover:bg-brand-700 text-white font-medium text-xs shadow-md transition-all transform active:scale-98"
              >
                {isGenerating ? (
                  <>
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="p-6 rounded-xl bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] shadow-sm flex flex-col justify-between min-h-[500px]">
            <div>
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/[0.06] pb-4 mb-4">
                <div>
                  <h3 className="text-base font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                ...
                {generatedContractText && (
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-white/[0.05] hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-brand-500/10 text-brand-600 dark:text-brand-400 hover:bg-brand-500/20 text-xs font-semibold border border-brand-500/15 transition-colors"
                    >
              ...
              {isGenerating ? (
                ...
                <div className="space-y-4">
                  <div className="p-4 rounded-lg bg-slate-900 text-slate-200 font-mono text-xs leading-relaxed overflow-x-auto whitespace-pre-wrap max-h-[450px] border border-white/[0.06] shadow-inner">
                    {generatedContractText}
                  </div>

                  {generatedDoc && (
                    <div className="p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-emerald-700 dark:text-emerald-300">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-medium text-xs shadow transition-all"
                        >
                          Ask RAG Assistant →
                        </button>
                      </div>
                ...
                <div className="py-24 text-center space-y-3">
                  <div className="w-14 h-14 rounded-lg bg-slate-100 dark:bg-white/[0.05] flex items-center justify-center text-slate-400 mx-auto">
                    <FileSignature className="w-7 h-7" />
                  </div>
                  <h4 className="text-sm font-medium text-slate-700 dark:text-slate-300">No Contract Drafted Yet</h4>
