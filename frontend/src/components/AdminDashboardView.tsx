...
export const AdminDashboardView: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {/* Header */}
      <div className="p-6 sm:p-8 rounded-xl bg-white dark:bg-gray-900 border border-slate-200 dark:border-white/[0.06]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-gray-900 p-5 rounded-lg border border-slate-200 dark:border-white/[0.06] space-y-2">
          <div className="flex items-center justify-between text-slate-400 text-xs font-medium uppercase">
            <span>Total Active Users</span>
            <Users className="w-4 h-4 text-brand-500" />
          </div>
          <div className="text-3xl font-semibold text-slate-900 dark:text-white">{analytics.total_users}</div>
        ...
        <div className="bg-white dark:bg-gray-900 p-5 rounded-lg border border-slate-200 dark:border-white/[0.06] space-y-2">
          ...
          <div className="text-3xl font-semibold text-slate-900 dark:text-white">{analytics.total_documents}</div>
        ...
        <div className="bg-white dark:bg-gray-900 p-5 rounded-lg border border-slate-200 dark:border-white/[0.06] space-y-2">
          ...
          <div className="text-3xl font-semibold text-slate-900 dark:text-white">{analytics.total_ai_requests.toLocaleString()}</div>
          <div className="text-[11px] text-emerald-500 font-medium flex items-center gap-1">
        ...
        <div className="bg-white dark:bg-gray-900 p-5 rounded-lg border border-slate-200 dark:border-white/[0.06] space-y-2">
          ...
          <div className="text-3xl font-semibold text-slate-900 dark:text-white">{(analytics.storage_used_mb / 1024).toFixed(2)} GB</div>
            ...
            {analytics.daily_query_trend.map((day, idx) => {
              ...
              return (
                <div key={idx} className="flex-1 flex flex-col items-center gap-2 h-full justify-end group">
                  <div className="text-[11px] font-mono font-medium text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity">
                    {day.queries}
                  </div>
                  <div
                  ...
                  ></div>
                  <div className="text-[11px] font-semibold text-slate-500">{day.date}</div>
                </div>
              );
            })}
          </div>
